...
namespace ServiceLibrary.Repositories
{
    public class SqlConnectionWrapperMock : Mock<ISqlConnectionWrapper>
    {
        ...
        public void SetupExecuteScalarAsyncFunc<T>(string sql, Dictionary<string, object> param, T result, Dictionary<string, object> outParameters = null)
        {
            Expression<Func<object, bool>> match = p => param == null || param.All(kv => Matches(kv.Value, SqlConnectionWrapper.Get<object>(p, kv.Key)));
            Setup(c => c.ExecuteScalarAsync<T>(sql, It.Is(match), It.IsAny<IDbTransaction>(), It.IsAny<int?>(), It.IsAny<CommandType?>()), result, outParameters);
        }
