...
module Storyteller {
    export class SystemTask extends UserTaskChildElement<ISystemTaskShape> implements ISystemTask, IUserTaskChildElement {
        ...
        public set persona(value: string) {
            ...
            if (valueChanged) {
                if (this.personaLabel) {
                    ...
                    this.shapesFactoryService.setSystemTaskPersona(value);
                }
        ...
        public render(graph: ProcessGraph, x: number, y: number, justCreated: boolean): IDiagramNode {
            ...
            this.insertVertex(graph, modelId, null, x - shift, y - (shapeHeight / 2) + this.ORIGIN_DIAMETER / 2, shapeWidth, shapeHeight, "shape=systemTask;strokeColor=none;fillColor=none;foldable=0;editable=0");

            // origin
            graph.graph.addCell(this.origin, this);

            // call-out
            graph.graph.addCell(this.callout, this);
            ...
            var cell = graph.graph.addCell(this.header, this.callout);
            ...
            cell = graph.graph.addCell(this.bodyCell, this.callout);

            // footer
            this.footerCell = graph.insertVertex(this.callout, "F" + modelId, null, 0.5, this.SYSTEM_TASK_HEIGHT - 34.5, this.SYSTEM_TASK_WIDTH - 1, 24, "shape=rectangle;foldable=0;strokeColor=none;fillColor=#FFFFFF;gradientColor=#DDDDDD;selectable=0");

            this.addOverlays(graph);

            this.commentsButton.render(graph, this.footerCell, this.footerCell.geometry.width - 116, 4, "shape=ellipse;strokeColor=none;fillColor=none;selectable=0");
            this.relationshipButton.render(graph, this.footerCell, this.footerCell.geometry.width - 92, 4, "shape=ellipse;strokeColor=none;fillColor=none;selectable=0");
            this.linkButton.render(graph, this.footerCell, this.footerCell.geometry.width - 68, 4, "shape=ellipse;strokeColor=none;fillColor=none;selectable=0");
            if (graph.viewModel.isReadonly && graph.viewModel.licenseType === Shell.LicenseTypeEnum.Viewer) {
                this.linkButton.disable();
            }
            this.mockupButton.render(graph, this.footerCell, this.footerCell.geometry.width - 44, 4, "shape=ellipse;strokeColor=none;fillColor=none;selectable=0");
            this.detailsButton.render(graph, this.footerCell, this.footerCell.geometry.width - 20, 4, "shape=ellipse;strokeColor=none;fillColor=none;selectable=0");
