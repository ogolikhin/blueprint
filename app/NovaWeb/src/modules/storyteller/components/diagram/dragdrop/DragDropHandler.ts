module Storyteller {
    ...
    export class DragDropHandler implements IDragDropHandler {
        ...
        public isValidDropSource(dropSource: MxCell) {
            ...
            var isDropSource: boolean = false;
            if (dropSource && dropSource.isVertex) {
                var diagramNodeElement = <IDiagramNodeElement>dropSource;
        ...
        public highlightDropTarget(me) {
            var state = this.layout.getDropEdgeState(this.getConvertedPoint(me));
            if (state != null) {
                if (!state.cell.edge) return;
                var cellId = Number((<IDiagramNodeElement>this.moveCell).getNode().getId());
        ...
        public createDragPreview() {
            ...
            this.dragPreview.style.border = 'dashed black 1px';
            ...
            this.dragPreview.style.background = "url('/Areas/Web/Style/images/Storyteller/draggable-icon-gray.png') no-repeat center center";
        ...
        private updateDragElt(isActive: boolean) {
            if (isActive) {
                this.dragPreview.style.background = "url('/Areas/Web/Style/images/Storyteller/draggable-icon-blue.png') no-repeat center center";
            } else {
                this.dragPreview.style.background = "url('/Areas/Web/Style/images/Storyteller/draggable-icon-gray.png') no-repeat center center";
            }
        }
        ...
        private installMouseDragDropListener() {
            ...
            this.graph.addMouseListener({
                ...
                mouseUp: function (sender, me) {
                    if (h.moveCell) {
                        var node = (<IDiagramNodeElement>h.moveCell).getNode();
                        if (h.currentState && h.currentState.cell.isEdge()) {
                            // drop
                            var edge = h.currentState.cell;
                            var cellId = Number(node.getId());
