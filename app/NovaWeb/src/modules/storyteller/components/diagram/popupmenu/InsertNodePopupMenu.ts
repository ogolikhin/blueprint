module Storyteller {
    ...
    export class InsertNodePopupMenu {
        ...
        private init() {
            this.graph = this.diagram.graph;

            this.installPopupMenuHandlers();

            this.graph.popupMenuHandler["setEnabled"](true);
            this.graph.popupMenuHandler["useLeftButtonForPopup"] = true;
        }

        private installPopupMenuHandlers() {
            this.graph.popupMenuHandler["isPopupTrigger"] = this.isPopupTrigger;

            this.graph.popupMenuHandler["popup"] = function (x, y, cell, evt) {

                /*
                 * this handler hooks the mxPopupMenu popup() function 
                 * it initiates a sequence of callbacks that create the
                 * UI for the popup menu and then show it at offsets x, y
                 */
        ...
        private updatePositionOfPopupMenu = (x: number, y: number) => {
            ...
            if (this.menu && this.menu["div"]) {
                ...
                // hide the popup and remove the listner if near the edges of the diagram, as otherwise it will go over other elements (e.g. the toolbar)
                if (
        ...
        private initScrollingPositionOfPopupMenu = () => {
            ...
            this.initialScrollPosition["x"] = this.diagram.container.scrollLeft;
            this.initialScrollPosition["y"] = this.diagram.container.scrollTop;
            ...
            let containerBoundingClientRect = this.diagram.container.getBoundingClientRect();
        ...
        private removeScrollHandler = () => {
            ...
            this.diagram.container.removeEventListener("scroll", this.scrollHandler);
        ...
        public createPopupMenu = (graph, menu, cell, evt) => {
            ...
            this.diagram.container.addEventListener("scroll", this.scrollHandler);
        };
        ...
        private isSourceNodeOfType(edge: MxCell, nodeType: NodeType) {
            ...
            if (edge && edge.source) {
                ...
                if (node.getNodeType() === NodeType.MergingPoint) {
                    let incomingLinks: DiagramLink[] = node.getIncomingLinks(this.diagram);
