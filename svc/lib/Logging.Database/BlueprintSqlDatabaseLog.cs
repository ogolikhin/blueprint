...
{
    /// <summary>
    /// Factories and helpers for using the <see cref="BlueprintSqlDatabaseSink"/>.
    /// </summary>
    public static class BlueprintSqlDatabaseLog
    {
        ...
        /// <param name="onCompletedTimeout">Defines a timeout interval for when flushing the entries after an <see cref="BlueprintSqlDatabaseSink.OnCompleted"/> call is received and before disposing the sink.
        ...
        public static SinkSubscription<BlueprintSqlDatabaseSink> LogToSqlDatabase(this IObservable<EventEntry> eventStream, string instanceName, string connectionString, string tableName = DefaultTableName, string storedProcedureName = DefaultStoredProcedureName, TimeSpan? bufferingInterval = null, int bufferingCount = Buffering.DefaultBufferingCount, TimeSpan? onCompletedTimeout = null, int maxBufferSize = Buffering.DefaultMaxBufferSize)
        {
            ...
            return new SinkSubscription<BlueprintSqlDatabaseSink>(subscription, sink);
        }
