...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public static insertClonedUserDecisionInternal(sourceIds: number[], destinationId: number, layout: ILayout, 
                                                                       shapesFactoryService: ShapesFactory, _userDecisionShape: IProcessShape): number {
        layout.setTempShapeId(layout.getTempShapeId() - 1);
        const userDecisionShape = shapesFactoryService.createModelUserDecisionShape(layout.viewModel.id,
            layout.viewModel.projectId, layout.getTempShapeId(), -1, -1);
        // COPY PROPERTIES
        userDecisionShape.name = _userDecisionShape.name; 
        userDecisionShape.personaReference = _.cloneDeep(_userDecisionShape.personaReference); 
        userDecisionShape.propertyValues = _.cloneDeep(_userDecisionShape.propertyValues); 
        ...
        // // add new branch
        // const branchDestination: IProcessShape = layout.getConditionDestination(userDecisionShape.id);
        // ProcessAddHelper.insertClonedUserDecisionConditionInternal(userDecisionShape.id, branchDestination.id, layout, shapesFactoryService);

        return userDecisionShape.id;
    }

    private static insertClonedUserDecisionConditionInternal(userDecisionId: number, branchDestinationId: number,
                                                       layout: ILayout, shapesFactoryService: ShapesFactory, label?: string): number {
        // add user task and system task shapes
        let userTaskShapeId = ProcessAddHelper.insertUserTaskInternal(layout, shapesFactoryService);
        let systemTaskId = ProcessAddHelper.insertSystemTaskInternal(layout, shapesFactoryService);
    ...
    public  static insertClonedSystemDecisionInternal(link: IProcessLink, layout: ILayout, 
                            shapesFactoryService: ShapesFactory, _systemDecisionShape: IProcessShape): number {
        const sourceId = link.sourceId;
        const destinationId = link.destinationId;
        layout.setTempShapeId(layout.getTempShapeId() - 1);
        const systemDecision = shapesFactoryService.createSystemDecisionShapeModel(layout.getTempShapeId(),
        ...
        // COPY PROPERTIES
        systemDecision.name = _systemDecisionShape.name; 
        systemDecision.personaReference = _.cloneDeep(_systemDecisionShape.personaReference); 
        systemDecision.propertyValues = _.cloneDeep(_systemDecisionShape.propertyValues); 
    ...
    private static insertClonedSystemDecisionConditionInternal(systemDecisionId: number, branchDestinationId: number, layout: ILayout,
                                                         shapesFactoryService: ShapesFactory, label?: string): number {
        let systemTaskId = ProcessAddHelper.insertSystemTaskInternal(layout, shapesFactoryService);
