...
namespace ArtifactStore.Repositories
{
    public class SqlReviewsRepository: IReviewsRepository
    {
        ...
        public async Task<ReviewArtifactIndex> GetReviewTableOfContentArtifactIndexAsync(int reviewId, int revisionId, int artifactId, int userId)
        {
            ...
            int refreshInterval = await GetRebuildReviewArtifactHierarchyInterval();
            var param = new DynamicParameters();
            param.Add("@reviewId", reviewInfo.Id);
            param.Add("@revisionId", revisionId);
            param.Add("@userId", userId);
            param.Add("@artifactId", artifactId);
            param.Add("@refreshInterval", refreshInterval);
            ...
            var result = await ConnectionWrapper.QueryAsync<ReviewArtifactIndex>("GetReviewTableOfContentArtifactIndex", param, commandType: CommandType.StoredProcedure);

            var resultValue = param.Get<int>("@retResult");
            if (resultValue == 1 || resultValue == 2)
            {
                ThrowReviewNotFoundException(reviewId, revisionId);
            }
        ...
        private async Task<RDReviewedArtifacts> GetReviewUserStatsXmlAsync(int reviewId, int userId)
        {
            ...
            var xmlString = (await ConnectionWrapper.QueryAsync<string>("GetReviewUserStatsXml", parameters, commandType: CommandType.StoredProcedure)).SingleOrDefault();
        ...
        private Task UpdateReviewUserStatsXmlAsync(int reviewId, int userId, RDReviewedArtifacts rdReviewedArtifacts)
        {
            ...
            return ConnectionWrapper.ExecuteAsync("UpdateReviewUserStatsXml", parameters, commandType: CommandType.StoredProcedure);
        }
