...
{
    /// <summary>
    /// Tests for the Action Handler Service Repository
    /// </summary>
    [TestClass]
    public class RepositoryTests
    {
        ...
        public async Task GetPropertyModificationsForRevisionIdAsync_CompletesSuccessfuly()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                result.Add(new SqlModifiedProperty {PropertyName = $"Property{i}", ArtifactId = 0, ItemId = 0, ProjectId = 0, Type = 0, TypeId = 0, VersionId = 0, StartRevision = 0, EndRevision = 0, NewPropertyValue = ""});
            }
            connectionMock.SetupQueryAsync(storedProcedure, parameters, result);

            //act
            var repository = new ActionHandlerServiceRepository(connectionMock.Object);
            ...
            var matchFound = repositoryResult.Any(r => firstResult.ArtifactId == r.ArtifactId && firstResult.EndRevision == r.EndRevision && firstResult.ItemId == r.ItemId && firstResult.NewPropertyValue == r.NewPropertyValue && firstResult.ProjectId == r.ProjectId && firstResult.PropertyName == r.PropertyName && firstResult.StartRevision == r.StartRevision && firstResult.Type == r.Type && firstResult.TypeId == r.TypeId && firstResult.VersionId == r.VersionId);
        ...
        public async Task GetWorkflowPropertyTransitionsForArtifactsAsync_CompletesSuccessfuly()
        {
            ...
            var parameters = new Dictionary<string, object> {{nameof(userId), userId}, {nameof(revisionId), revisionId}, {nameof(eventType), eventType}, {nameof(itemIds), SqlConnectionWrapper.ToDataTable(itemIds)}};
        ...
        public async Task GetWorkflowStatesForArtifactsAsync_CompletesSuccessfuly()
        {
            ...
            var parameters = new Dictionary<string, object> {{nameof(userId), userId}, {nameof(artifactIds), SqlConnectionWrapper.ToDataTable(artifactIds)}, {nameof(revisionId), revisionId}, {nameof(addDrafts), addDrafts}};
