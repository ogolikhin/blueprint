...
namespace SearchEngineLibrary.Helpers
{
    ...
    {
        public static string GetArtifactIdsQuery(int scopeId, Pagination pagination, bool includeDraft, int userId)
        {
            ...
            query.Append("IF(@includeDraft = 0) BEGIN INSERT INTO #VersionArtifactId SELECT col.[VersionArtifactId] FROM [dbo].[CollectionAssignmentVersions] as col JOIN [dbo].[ItemVersions] as iv on iv.[HolderId] = col.[VersionArtifactId] CROSS APPLY [dbo].[Getartifactpermission](@userId, iv.VersionProjectId, iv.VersionArtifactId) as p ");
            ...
            query.Append("END ELSE BEGIN INSERT INTO #VersionArtifactId SELECT col.[VersionArtifactId] FROM [dbo].[CollectionAssignmentVersions] as col JOIN [dbo].[ItemVersions] as iv ON iv.HolderId = col.VersionArtifactId CROSS APPLY [dbo].[Getartifactpermission](@userId, iv.VersionProjectId, iv.VersionArtifactId) as p ");
