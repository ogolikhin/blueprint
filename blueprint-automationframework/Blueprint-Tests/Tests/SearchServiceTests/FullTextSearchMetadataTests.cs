...
namespace SearchServiceTests
{
    ...
    public class FullTextSearchMetadataTests : TestBase
    {
            ...
            SearchServiceTestHelper.ProjectRole projectRole)
        {
            ...
            // Create user with project role permissions to only 1 project
        ...
        [TestCase(0, 0, SearchServiceTestHelper.ProjectRole.None)]
        [TestRail(182376)]
        [Description("Search for artifact in multiple projects with user with varying permissions to a single artifact. Executed search must return search metadata" +
                     " result that indicates whether the user was able to see the artifact in the search results or not.")]
        public void FullTextSearchMetadata_UserWithProjectRolePermissionsToSingleArtifact_VerifyResultIndicatesResultsFromSingleArtifact(
            int expectedHitCount,
            int expectedTotalPageCount,
            SearchServiceTestHelper.ProjectRole projectRole)
        {
        ...
        public void FullTextSearchMetadata_SearchMetadataWithInvalidSession_401Unauthorized(BaseArtifactType artifactType, string invalidAccessControlToken)
        {
            ...
            // Create user with author project role to projects
            var userWithProjectRole = SearchServiceTestHelper.CreateUserWithProjectRolePermissions(
                Helper,
