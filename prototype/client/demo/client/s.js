 ...
 * @author pegah
 */
var bpApp = angular.module('bpApp',['ngCookies','ngRoute','ui.bootstrap','ngJScrollPane'])
...
bpApp.controller('DeviceController', function($scope, $rootScope, $timeout, $http, $location, $cookies,  $cookieStore, $routeParams, MetaService, HiLite) {    
	...
	$scope.title = "TI Resource Explorer";
	$scope.keywords = "Devices, Development Tools, Libraries";
	$scope.description = "Access examples, libraries, executables and documentation for your Texas Instruments device and development board";
	
	if (typeof($scope.deviceId) != "undefined") {
		$scope.title = $scope.deviceId + " | " + $scope.title;
		$scope.keywords = $scope.deviceId + ", Libraries, executables, examples, documents";
		$scope.description = "Access examples, libraries, executables and documentation for " + $scope.deviceId;
	}
	if (typeof($scope.devtoolId) != "undefined") {
		$scope.title = $scope.devtoolId + " | " + $scope.title;
		$scope.keywords = $scope.devtoolId + ", Libraries, executables, examples, documents";
		$scope.description = "Access examples, libraries, executables and documentation for " + $scope.devtoolId;
	}
	...
	*/
	if ($location.absUrl().indexOf("localhost") == -1) {
		...
		$http({
			url: "http://" + $location.host() + ":" + $location.port() + "/analytics",
			method: "GET"
		}).success(function(data, status, headers, config) {
			//call our server with this sessionId
			$http.post("api/storeanalyticstate", data, {headers:{"Content-Type":"text/plain;charset=UTF-8"}}).success(function(data, status, headers, config) {
				//console.log('success - passed sessionId: '+ $scope.ideSessionId );
			}).error(function(data, status, headers, config) {
				console.log("failed to connect to api/storeanalyticstate");
			});	
		}).error(function(data, status, headers, config) {
			console.log("failed to connect to https://" + $location.host() + ":" + $location.port() + "/analytics");
		});
	}
    ...
    $scope.openLink = function(currentNode, childNode, type) {
		...
		if ($scope.selectedTreeNode.parentContent != null) { // && scope.selectedNode.parentContent.resourceType=='file') {	
			...
			if ($scope.selectedTreeNode.parentContent.text != undefined) {
				$scope.title = $scope.selectedTreeNode.parentContent.text + " | " + "TI Resource Explorer";
			}
			...
			if ($scope.selectedTreeNode.parentContent.tags != undefined) {
				$scope.keywords = $scope.selectedTreeNode.parentContent.tags + ", Libraries, executables, examples, documents";
			}
								...
								if (typeof( n.parentUrl) != "undefined" && n.parentUrl != null && n.parentUrl !== 'api') {  
									...
									$http({
										...
										if (scope.selectedNode.parentContent.overviewDescription != null && scope.selectedNode.parentContent.overviewDescription != 'undefined')  {
											scope.title = scope.selectedNode.parentContent.text + " | " + "TI Resource Explorer";
											scope.keywords = scope.selectedNode.parentContent.text+ ", Libraries, executables, examples, documents";
					...
					$http({
						...
						if (scope.selectedNode.parentContent != null) { // && scope.selectedNode.parentContent.resourceType=='file') {
							...
							if (scope.selectedNode.parentContent.tags != undefined) {
								scope.keywords = scope.selectedNode.parentContent.tags + ", Libraries, executables, examples, documents";
							}
							scope.$root.metaservice.set(scope.title,scope.description,scope.keywords);	
							if (scope.selectedNode.parentContent.resourceType ==='file' ||
								scope.selectedNode.parentContent.resourceType == 'project.energia') {
