export class IconRack {
    ...
    private createImageShape(container: HTMLElement) {
        var bounds = this.rectangle(0, 0, 16, 16);
    ...
    private getInfoIconRackBounds(state: MxCellState) {
        ...
        return this.rectangle(state.x + state.width - w * s, state.y - h * s - 5, w * s, h * s);
    }
    private rectangle(x: number, y: number, width: number, height: number): MxRectangle {
