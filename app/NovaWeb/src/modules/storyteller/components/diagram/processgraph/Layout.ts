module Storyteller {
    ...
    export interface ILayout {
        ...
        insertSystemDecision(connector: DiagramLink);
    ...
    export class Layout implements ILayout {
        private graph: ProcessGraph;
        private model: IStorytellerViewModel;
        ...
        constructor(
            graph: ProcessGraph,
            model: IStorytellerViewModel,
            rootScope: any,
            private shapesFactoryService: ShapesFactoryService,
            private messageService: Shell.IMessageService,
            private $log: ng.ILogService
        ) {
            this.init(graph, model, rootScope);
        }

        public init(graph: ProcessGraph, model: IStorytellerViewModel, rootScope: any) {
            this.graph = graph;
            this.model = model;
            this.rootScope = rootScope;

            this.graph.graph.cellRenderer = new ProcessCellRenderer();
        ...
        private dumpDebugInformation(): void {
            if (window.console && console.log) {
                for (let s of this.model.shapes) {
                ...
                for (let b of this.model.decisionBranchDestinationLinks) {
        ...
        public render(useAutolayout: boolean, selectedNodeId: number) {
            ...
            this.model.updateTreeAndFlows();

            if (useAutolayout) {
                this.preprocessor = new GraphLayoutPreprocessor(this.model, this.messageService);
                this.preprocessor.setCoordinates();
            }
            
            let validationErrors: string[] = [];
            if (!ProcessValidator.isValid(this.model, this.rootScope, validationErrors)) {
            ...
            for (i in this.model.links) {
                var link: IProcessLinkModel = this.model.links[i];
                if (linksMap[link.destinationId.toString()] == null) {
                    linksMap[link.destinationId.toString()] = [];
                }

                link.parentId = this.model.id;
                linksMap[link.destinationId.toString()].push(link);
            }

            const nodeFactorySettings = new NodeFactorySettings();
            nodeFactorySettings.isCommentsButtonEnabled = !this.model.isHistorical && !this.model.isSMB;
            nodeFactorySettings.isRelationshipButtonEnabled = !this.model.isHistorical && !this.model.isSMB;
            nodeFactorySettings.isDetailsButtonEnabled = this.model.isSpa;
            nodeFactorySettings.isLinkButtonEnabled = this.model.isSpa && !this.model.isHistorical;
            nodeFactorySettings.isMockupButtonEnabled = this.model.isSpa;
            nodeFactorySettings.isPreviewButtonEnabled = this.model.isSpa && !this.model.isHistorical;

            graphModel.beginUpdate();

            try {
                for (i in this.model.shapes) {
                    var shape: IProcessShape = this.model.shapes[i];
                    ...
                    if (node != null) {
                        node.render(this.graph, this.getXbyColumn(node.column), this.getYbyRow(node.row),
                            this.model.isShapeJustCreated(shape.id));

                        // Hide system tasks for process types different than 'Business Process'
                        if (this.model.propertyValues["clientType"].value !== ProcessType.UserToSystemProcess) {
                            if (node.getNodeType() == NodeType.SystemTask) {
                                (<SystemTask>node).setCellVisible(this.graph.graph, false);
                            }
                            if (node.getNodeType() === NodeType.SystemDecision) {
                                (<SystemDecision>node).hideMenu(this.graph);
                            }
                        }
                    }
                }
                ...
                for (i in linksMap) {
                    ...
                    if (linkArray.length === 1) {
                        ...
                        var mergingPointShape = this.shapesFactoryService.createModelMergeNodeShape(this.model.id, this.model.projectId, --tempShapeId, destinationNode.column - 1, destinationNode.row);
                ...
                let edges: DiagramLink[] = this.graph.graph.getChildEdges(this.graph.getDefaultParent());
                for (let j of edges) {
                    j.renderLabel();
                }
                ...
                this.graph.graph.orderCells(false, this.graph.graph.getChildVertices(this.graph.graph.getDefaultParent()));

                this.model.resetJustCreatedShapeIds();
        ...
        public scrollShapeToView(shapeId: string) {
            ...
            if (node) {
                (<any>this.graph.graph).scrollCellToVisible(node, true);
            }
        }

        public getDropEdgeState(mouseCoordinates: MxPoint): any {
            var view: MxGraphView = this.graph.graph.getView();
        ...
        public updateBranchDestinationId(oldDestinationId: number, newDestinationId: number) {
            this.model.getDecisionBranchDestinationLinks((link: IProcessLink) => {
        ...
        public insertTaskWithUpdate = (edge: MxCell): void => {
            ...
            if (this.model.isWithinShapeLimit(2)) {
        ...
        public getConditionDestination(decisionId: number): IProcessShape {
            if (this.model.isDecision(decisionId)) {
                let conditionShapeIds: number[] = this.model.getNextShapeIds(decisionId);
        ...
        public insertSystemDecisionConditionWithUpdate = (decisionId: number, label?: string, conditionDestinationId?: number): number => {
            ...
            // system task
            if (this.model.isWithinShapeLimit(1)) {
        ...
        public insertUserDecision = (edge: MxCell) => {
            ...
            if (this.model.isWithinShapeLimit(5)) {
                let sourcesAndDestinations = this.getSourcesAndDestinations(edge);
        ...
        private getNextSystemTaskOnMainCondition(id: number): number {
            let nextIds = this.model.getNextShapeIds(id);
            if (nextIds && nextIds.length > 0) {
                let nextMainConditionId = nextIds[0];
                if (this.model.getShapeTypeById(nextMainConditionId) === ProcessShapeType.SystemTask) {
        ...
        public handleUserTaskDragDrop(userTaskShapeId: number, edge: MxCell) {
            let oldAfterShapeId: number = this.model.getFirstNonSystemShapeId(userTaskShapeId);
            let oldBeforeShapeIds = this.model.getPrevShapeIds(userTaskShapeId);
        ...
        private getSourcesAndDestinations(edge: MxCell): SourcesAndDestinations {
            ...
            if (source.getNodeType() === NodeType.MergingPoint) {
                sourceIds = this.model.getPrevShapeIds(destinationId);
            }
        ...
        private updateBranchDestination(processLink: IProcessLink) {
            ...
            if (this.model.decisionBranchDestinationLinks == null) {
                this.model.decisionBranchDestinationLinks = new Array<IProcessLink>();
                this.model.decisionBranchDestinationLinks.push(processLink);
            }
            else {
                var matchingLinks = this.model.getDecisionBranchDestinationLinks((link: IProcessLink) => {
        ...
        private getBranchDestinationIds(decisionId: number): number[] {
            if (this.model.decisionBranchDestinationLinks == null) {
                ...
                return [Number(this.model.getEndShapeId())];
            }

            return this.model.decisionBranchDestinationLinks
        ...
        private getDecisionConditionDestination(shapeId: number): IProcessShape {
            let nextShapeIds: number[] = this.model.getNextShapeIds(shapeId);

            if (nextShapeIds && nextShapeIds.length > 0) {
                let nextShapeId: number = nextShapeIds[0];
                let shapeType: ProcessShapeType = this.model.getShapeTypeById(shapeId);

                if (shapeType === ProcessShapeType.SystemTask) {
                    return this.model.getShapeById(nextShapeId);
                } else {
                    return this.getDecisionConditionDestination(nextShapeId);
                }
            }

            return this.model.getShapeById(shapeId);
        }

        private insertUserTaskInternal() {
            var userTaskShape = this.shapesFactoryService.createModelUserTaskShape(this.model.id, this.model.projectId, --tempShapeId, -1, -1);
        ...
        private insertSystemTaskInternal() {
            var systemTaskShape = this.shapesFactoryService.createModelSystemTaskShape(this.model.id, this.model.projectId, --tempShapeId, -1, -1);
        ...
        private insertSystemDecisionInternal(link: IProcessLink): number {
            ...
            var systemDecision = this.shapesFactoryService.createSystemDecisionShapeModel(--tempShapeId, this.model.id, this.model.projectId, -1, -1);
        ...
        private insertSystemDecisionConditionInternal(systemDecisionId: number, branchDestinationId: number, label?: string): number {
            ...
            let orderIndex: number = this.model.getNextOrderIndex(systemDecisionId);
        ...
        private insertUserDecisionInternal(sourceIds: number[], destinationId: number): number {
            var userDecisionShape = this.shapesFactoryService.createModelUserDecisionShape(this.model.id, this.model.projectId, --tempShapeId, -1, -1);
            ...
            var nextShapeType = this.model.getShapeTypeById(destinationId);

            if (nextShapeType === ProcessShapeType.End || this.model.hasMultiplePrevShapesById(destinationId)) {
        ...
        private insertUserDecisionConditionInternal(userDecisionId: number, branchDestinationId: number, label?: string): number {
            ...
            let systemTaskId = this.insertSystemTaskInternal();
            let orderIndex = this.model.getNextOrderIndex(userDecisionId);
        ...
        private addShape(processShape: IProcessShape): void {
            if (processShape != null) {
                this.model.shapes.push(processShape);
                this.model.addJustCreatedShapeId(processShape.id);
            }
        }
        ...
        private addLinkInfo(sourceId: number, destinationId: number, orderIndex: number = 0, label: string = "",
            ...
            var link = new ProcessLinkModel(this.model.id, sourceId, destinationId, orderIndex, label, source, destination);
            this.model.links.push(link);
            return link;
        }

        private updateLink(sourceId: number, oldDestinationId: number, newDestinationId: number) {
            var index: number = this.model.getLinkIndex(sourceId, oldDestinationId);
            if (index > -1) {
                this.model.links[index].destinationId = newDestinationId;
            }
        }
        ...
        private deleteLink(sourceId: number, destinationId: number): void {
            var links = this.model.links.filter(link => {
