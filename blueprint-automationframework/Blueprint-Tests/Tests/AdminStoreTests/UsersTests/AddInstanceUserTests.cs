...
namespace AdminStoreTests.UsersTests
{
    ...
    public class AddInstanceUserTests : TestBase
    {
        ...
        [TestCase(null)]
        [TestCase((UserSource)0xFF)]
        [TestCase(UserSource.Unknown)]
        [TestCase(UserSource.Windows)]
        [Description("Create an instance user with a missing or Invalid user source. Add the user. " +
        ...
        {
            // Setup:
            var createdUser = AdminStoreHelper.GenerateRandomInstanceUser(source: source);
