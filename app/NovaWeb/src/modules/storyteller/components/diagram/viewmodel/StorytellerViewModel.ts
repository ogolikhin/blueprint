module Storyteller {
    /**
    * Storyteller ViewModel Class 
    *
    * The StorytellerViewModel class wraps Process and ProcessClientModel
    * classes. It provides a higher level API for accessing the process
    * data and indexing structures. This viewmodel also contains the business 
    * logic for modifying the application's state.
    */

    export class StorytellerViewModel implements IStorytellerViewModel {
        private DEFAULT_SHAPE_LIMIT: number = 100;
        private _header: IStorytellerHeader = null;
        private _rootScope: any = null;
        private _scope: any = null;
        private _messageService: any = null;
        ...
        private _licenseType: Shell.LicenseTypeEnum;
        ...
        constructor(process, header?: IStorytellerHeader, scope?: any, messageService?: Shell.IMessageService) {
        ...
        public set licenseType(value: Shell.LicenseTypeEnum) {
        ...
        public updateProcessClientModel(process: IProcess) {
        ...
        private getPropertyValue(process: IProcess, propertyName: string) {
        ...
        public get isUserToSystemProcess(): boolean {
            return this.propertyValues["clientType"].value === ProcessType.UserToSystemProcess;
        }

        private initHeader() {
            /**
             * Initializes a header object that can be used to bind to the header
             * template. Note: if the header object is a service or value injected
             * into the header directive there will be a lifetime discrepancy 
             * between the header object (singleton) and this viewmodel. 
             */

            if (this._header) {
                this._header.init(this.typePrefix, this.id, this.name, this.isChanged, this.showLock, this.showLockOpen,
                    this.description, true, true, this.isUserToSystemProcess, this.isReadonly);
            }
        }
        ...
        public isWithinShapeLimit(additionalShapes: number = 1, isLoading: boolean = false): boolean {
            ...
            if (shapeCount < eightyPercent) {
                ...
                let message: string;
                let messageType: Shell.MessageType = Shell.MessageType.Error;
                if (isLoading) {
                ...
                if (message) {
                    ...
                    message = message.replace("{1}", this.shapeLimit.toString());
                    this.showMessage(Shell.MessageType.Warning, message);
                }
                return true; 
            }
        ...
        public showMessage(messageType: Shell.MessageType, messageText: string) {

            var message = new Shell.Message(messageType, messageText);
        ...
        public get links(): IProcessLinkModel[] {
            return <IProcessLinkModel[]>this.processClientModel.links;
        }
